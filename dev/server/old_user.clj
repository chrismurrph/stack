...
;; Above was mine. I suppose there's no reason filter wouldn't keep them in order
;;
(defn filter-keeping-order [delim lst]
...
(defn x []
  (letfn [(sum [term a next-term b]
    ...
    (let [build-fn (fn [multipliers powers]
                     (fn [x]
                       (reduce +
                               (map
                                 #(* %1 (Math/pow x %2))
          ...
          ;input (line-seq (java.io.BufferedReader. *in*))
          input ["1 2" "0 1" "2 20"]
          as (str->ints (first input))
          bs (str->ints (second input))
          third-row-ints (str->ints (nth input 2))
...
;;
;; 1 count outer
;; 2 find number of inners
;; 3 map over them to get inside dots for each as []
;; 4 so sum those dots and add to count from 1
;;
(defn x-33 []
  (let [outside-dots (fn [n] (* (dec n) 5))
        ...
        all-dots (fn [n]
                   (if (= n 1)
                     ...
                     (let [count-outer (outside-dots n)
                           ...
                           many-inner-dots-not-double-counted (map inside-dots inners)
        ...
        res (map all-dots nums)
        ]
    (doseq [v res]
      (println (str v))
      )))

;(defn rotate [in-str n]
;  (concat (drop n in-str) (take n in-str)))

(defn x-34 []
  (let [strings->line (fn [strings] (apply str (interpose " " strings)))
        rotate (fn [in-str]
                 (fn [n]
                   (concat (drop n in-str) (take n in-str))))
        all-rotations (fn [in-str]
                        (let [rotate-fn (rotate in-str)
                              num-rotations (dec (count in-str))
                              ;_ (println num-rotations)
                              res (map (fn [n] (rotate-fn (inc n))) (range num-rotations))
                              as-strings (mapv #(apply str %) res)
                              ;res ["abc" "def"]
                              ;_ (println res)
                              ]
                          (conj as-strings in-str)))
        make-line (fn [string]
                    (strings->line (all-rotations string)))
        ;lines (line-seq (java.io.BufferedReader. *in*))
        lines ["5"
               "abc"
               "abcde"
...
(defn x-36 []
  (let [
        ...
        line-a (second lines)
        line-b (nth lines 3)
        nums-a (str->ints line-a)
        nums-b (str->ints line-b)
        freq-a (frequencies nums-a)
        freq-b (frequencies nums-b)
        res (sort (keys (filter (fn [[k v]]
                                  (let [freq-in-a (get freq-a k)]
                                    (not= freq-in-a v))) freq-b)))
        ]
    (println (apply str (interpose " " res)))))

(defn x-37 []
  (let [intersection (fn [s1 s2] (if (< (count s2) (count s1))
                                   (recur s2 s1)
        ...
        get-counts (fn [pair]
                     (let [d1 (divisors (first pair))
                           d2 (divisors (second pair))]
                       (count (intersection d1 d2))))
...
(defn fibo-seq
  ([limit]
   (mod (nth (map first (iterate (fn fib-step [[a b]] [b (+ a b)]) [0N 1])) limit) 100000007)))
...
(defn x-39
  ...
  (let [str->ints (fn [string]
                    (map #(Integer/parseInt %)
                         (clojure.string/split string #" ")))
        fibo-seq (fn [limit] (mod (nth (map first
                                            (iterate
        ...
        input ["5"
               ...
               "10"
               "100"]
        nums (map #(Integer/parseInt %) (drop 1 input))
        _ (println nums)
        results (map fibo-seq nums)
        ]
    (doseq [v results]
      (println (str v))
      )))
...
(defn x-40 []
  (let [str->ints (fn [string]
        ...
        combinations (fn [pop sz]
                       (/ (factorial pop) (* (factorial sz) (factorial (- pop sz)))))
        ;input (line-seq (java.io.BufferedReader. *in*))
        input ["1"
               "65 44"]
        nums (map str->ints (drop 1 input))
        ;_ (println nums)
        results (map (fn [[pop sz]] (mod (combinations pop sz) 100000007)) nums)]
    (doseq [v results]
      (println (str v))
      )))
...
(defn x-41 []
  (letfn [(power-add [target-sum power row-of-nums]
    ...
    (let [power-adder-decider (fn [power coll target-sum]
          ...
          ;;
          ;; Failing because don't know how to limit the number of possibilities when
          ;; the numbers have to be squared.
          ;;
          ...
          _ (println "all to try: " (count all-to-try))
          biggest-first-for-each (map #(sort > %) all-to-try)
          _ (println "now each list individually sorted so going down")
          most-numerous-first (sort-by (comp - count) biggest-first-for-each)
          ;_ (println "all to try: " (count most-numerous-first))
          triaged (triage-down power most-numerous-first target-sum 0)
          _ (println "triage completed")
...
(defn x-42 []
  (let [
        ;input (line-seq (java.io.BufferedReader. *in*))
        input ["3"]
        count (Integer/parseInt (first input))
        ]
    (doseq [x (range count)]
      (println "Hello World"))))
...
(defn filter-me [f lst]
  (loop [head (first lst) tail (rest lst) acc []]
    (if-not (nil? head)
      (do
        (if (f head)
...
(defn x-43 []
  (let [res (filter-me #(< % 3) [1 2 3 4 5])]
    (doseq [x res]
      (println x))))

(defn reverse-me [list]
  (loop [current-lst list acc ()]
    (if (seq current-lst)
      (let [head (first current-lst)]
        (recur (rest current-lst) (conj acc head)))
      acc)))
...
(defn my-count [list]
  (loop [current-lst list acc 0]
    (if (seq current-lst)
      (let [head (first current-lst)]
        (recur (rest current-lst) (inc acc)))
      acc)))
...
(defn my-abs [list]
  (let [abs (fn [val]
    ...
    (loop [current-lst list acc []]
      (if (seq current-lst)
        (let [head (first current-lst)]
          (recur (rest current-lst) (conj acc (abs head))))
        acc))))
...
(defn x []
  (let [round (fn [precision d] (let [factor (Math/pow 10 precision)]
                                  (/ (Math/round (* d factor)) factor)))
        factorial (fn [n] (reduce * (range 1N (inc n))))
        calc-item (fn [n x]
                    (let [numerator (Math/pow x n)
                          denom (factorial n)]
                      (/ numerator denom)))
        calc-for-ten (fn [x]
                       (reduce
                         (fn [acc ele]
                           (let [valu (calc-item ele x)]
                             (+ acc valu)))
                         0
                         (range 10)))
        ;input (line-seq (java.io.BufferedReader. *in*))
        ...
        results (map (comp round-it calc-for-ten) numbers)
        ]
    (doseq [x results]
      (println x))))